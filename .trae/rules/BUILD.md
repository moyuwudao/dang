...
# BUILD.md - 构建流程规范（唯一构建规则源）

> **本文件是构建的唯一真相来源。所有构建操作均参照此文件。**
> **新增/修改 SKILL 时，应引用此文件而非复制内容。**

---

## 核心理念

构建是将代码变成可运行程序的过程。构建红线是**绝对不能违反**的规则。
...
| 检查项 | 说明 | 验证方式 |
|-------|------|---------|
| 1. 同步代码 | 构建前必须同步代码到 WSL | rsync 输出含 `sent` 和 `total size` |
| 2. WSL 环境构建 | 必须在 WSL `dang` 实例中构建 | 输出含 `✓ Built build/app/outputs/flutter-apk/app-release.apk` |
| 3. WSL cp 复制 | 必须使用 WSL cp 命令复制 APK | 禁止 `Copy-Item` 或 `copy` |
| 4. 时间戳命名 | 必须生成 `changji_app_YYYYMMDD_HHMM.apk` | 文件名匹配 `changji_app_\d{8}_\d{4}\.apk` |
| 5. 验证 APK | 必须检查文件修改时间 | LastWriteTime 为当前时间 ±5 分钟 |
...
```powershell
# 第1步：同步代码到 WSL（必须！同步所有相关文件，rsync 设 60 秒超时）
wsl -d dang bash -c "rsync -av --timeout=60 --delete /mnt/d/trae_projects/dang/lib/ /home/mayn/dang/lib/ && rsync -av --timeout=60 --delete /mnt/d/trae_projects/dang/android/ /home/mayn/dang/android/ && rsync -av --timeout=60 /mnt/d/trae_projects/dang/pubspec.yaml /home/mayn/dang/pubspec.yaml && rsync -av --timeout=60 --delete /mnt/d/trae_projects/dang/assets/ /home/mayn/dang/assets/"

# 第2步：WSL 环境构建（20 分钟超时）
wsl -d dang bash -c "export PUB_HOSTED_URL=https://pub.flutter-io.cn && export FLUTTER_STORAGE_BASE_URL=https://storage.flutter-io.cn && export PATH=/home/mayn/flutter/bin:/usr/bin:/bin:/usr/local/bin:\$PATH && cd /home/mayn/dang && flutter clean && timeout 1200 flutter build apk --release"
...
```powershell
# 完整清理（pub get 设 5 分钟超时）
wsl -d dang bash -c "cd /home/mayn/dang && flutter clean && rm -rf .dart_tool && timeout 300 flutter pub get"
```
...
| 类型 | 情况 | 原因 |
|-----|------|------|
| 🤖 自动构建 | Bug 修复、UI 调整、新功能 | 需要设备验证 |
| ❓ 询问用户 | 代码重构、配置修改、文档更新 | 不影响功能，不需要立即验证 |
...
当检测到构建操作时触发阻断，必须按顶部「构建强制检查」清单逐项验证后才继续。
...
# 第3步：生成带时间戳的 APK 文件名（格式：changji_app_YYYYMMDD_HHMM.apk）
$timestamp = Get-Date -Format "yyyyMMdd_HHmm"
$apkName = "changji_app_${timestamp}.apk"
