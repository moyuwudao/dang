      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_reportContent.isEmpty && !_isGenerating) _buildEmptyState(theme),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('时间范围', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
      ...
      children: [
        Row(
          children: [
            ...
            const Text('周报内容', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const Spacer(),
            IconButton(
  ...
  Widget _buildEmptyState(ThemeData theme) {
    return Center(
      child: Padding(
        ...
        child: Column(
          children: [
            Icon(Icons.summarize_outlined, size: 64, color: AppColors.textTertiary),
            const SizedBox(height: 16),
            Text('选择时间范围，一键生成周报', style: TextStyle(color: AppColors.textTertiary, fontSize: 16)),
            const SizedBox(height: 8),
            Text('AI将自动汇总该时间段内的所有记录', style: TextStyle(color: AppColors.textTertiary, fontSize: 13)),
          ],
        ),
      ),
    );
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      if (apiConfig == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('请先配置API Key'), backgroundColor: AppColors.error),
          );
        }
        return;
      }
      ...
      final filteredRecords = allRecords.where((r) {
        ...
        return !created.isBefore(_dateRange.start) && !created.isAfter(_dateRange.end.add(const Duration(days: 1)));
      }).toList();

      if (filteredRecords.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('该时间段内没有记录'), backgroundColor: AppColors.warning),
          );
        }
        return;
      }
      ...
      buffer.writeln('以下是 ${_dateRange.start.month}月${_dateRange.start.day}日 到 ${_dateRange.end.month}月${_dateRange.end.day}日 的工作记录：');
      buffer.writeln();
      ...
      for (int i = 0; i < filteredRecords.length; i++) {
        ...
        if (r.content != null && r.content!.isNotEmpty) {
          final content = r.content!.length > 500 ? '${r.content!.substring(0, 500)}...' : r.content!;
          buffer.writeln(content);
        }
        if (r.aiAnalysisResults.isNotEmpty) {
          buffer.writeln('[AI分析摘要]');
          for (final analysis in r.aiAnalysisResults) {
            final summary = analysis.content.length > 200 ? '${analysis.content.substring(0, 200)}...' : analysis.content;
...
请语言简洁，重点突出，适合发给投资人或合作伙伴。''';
      ...
      final customTemplate = templateConfig.customTemplates
          .where((t) => t.id == templateConfig.defaultWeeklyReportTemplateId && t.type == 'weekly_report')
          .firstOrNull;
      if (customTemplate != null) {
        systemPrompt = customTemplate.systemPrompt;
      } else if (templateConfig.defaultWeeklyReportTemplateId == 'builtin_weekly_executive') {
...
请极度简洁，每部分不超过5条，适合3分钟阅读完毕。''';
      } else if (templateConfig.defaultWeeklyReportTemplateId == 'builtin_weekly_detailed') {
...
请详细、结构化，适合存档和复盘。''';
  ...
  Future<void> _saveReport() async {
    ...
    final nameController = TextEditingController(
      text: '${_dateRange.start.month}月${_dateRange.start.day}日-${_dateRange.end.month}月${_dateRange.end.day}日周报',
    );
    ...
    if (confirmed == true && nameController.text.trim().isNotEmpty) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('周报已保存'), backgroundColor: AppColors.success),
        );
      }
    }
  }
      ...
      children: [
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            const Text('已保存的周报', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            const Spacer(),
            Text('${_savedReports.length} 份', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
          ],
        ),
        const SizedBox(height: 8),
        if (_savedReports.isEmpty)
          Container(
            ...
            child: const Text('暂无保存的周报', style: TextStyle(color: AppColors.textSecondary)),
          )
        else
