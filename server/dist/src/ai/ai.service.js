...
let AiService = class AiService {
    constructor(httpService, apiKeyService, subscriptionService, redisService, apiUsageLogRepository, planApiPolicyRepository) {
    ...
    async chat(userId, params) {
        ...
        if (!subscription.data || subscription.data.status !== 'active') {
            throw new common_1.HttpException('订阅已过期，请充值', 402);
        }
        ...
        if (balance < estimatedQuota) {
            throw new common_1.HttpException('配额不足，请充值', 402);
        }
        try {
            ...
            const quotaConsumed = await this.calculateQuotaConsumed(userId, apiKey.provider, params.model || 'default', totalTokens);
...
exports.AiService = AiService = __decorate([
    ...
    __param(5, (0, typeorm_1.InjectRepository)(plan_api_policy_entity_1.PlanApiPolicy)),
