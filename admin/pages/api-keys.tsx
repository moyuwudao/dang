...
import { Plus, Search, Trash2, Key, Eye, Copy, Check, Shield, Sparkles, AlertCircle } from 'lucide-react';
...
export default function ApiKeysPage() {
  ...
  const handleAddKey = async () => {
    if (!newKey.model.trim() || !newKey.apiKey.trim()) return;
    setSubmitting(true);
    try {
      ...
      setNewKey({ provider: 'qwen', model: '', apiKey: '', rateLimitPerMin: 60 });
    } catch (err: any) {
                ...
                <TableBody emptyContent="暂无 API Key">
                  {filteredKeys.map((key) => (
                    <TableRow key={key.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <TableCell>
                        <span className="text-gray-500">{formatDate(key.createdAt)}</span>
                      </TableCell>
                      <TableCell>
        ...
        >
          ...
          <ModalBody>
            <div className="space-y-4">
              ...
              >
                <SelectItem key="qwen" value="qwen">
                  阿里云通义千问
                </SelectItem>
              </Select>
              <Input
          ...
          <ModalFooter>
            ...
            <Button
              ...
              isDisabled={!newKey.model.trim() || !newKey.apiKey.trim()}
            >
              创建
            </Button>
          </ModalFooter>
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-gray-50/80 rounded-xl">
                    ...
                    <Chip
                      color={selectedKey.isActive ? 'success' : 'danger'}
                      size="sm"
                      className={
                  ...
                  <div className="p-4 bg-gray-50/80 rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                      <p className="font-mono text-sm bg-gray-200/50 px-2 py-1 rounded truncate max-w-[120px]">
                        {selectedKey.id}
                      </p>
                      <Button
                        size="sm"
        ...
        >
          ...
          <ModalBody>
            <p className="text-gray-600">
              确定要删除 <span className="font-semibold text-gray-800">{selectedKey?.provider}</span> 的{' '}
              <span className="font-semibold text-gray-800">{selectedKey?.model}</span> API Key 吗？
            </p>
          </ModalBody>
          <ModalFooter>
            <Button
